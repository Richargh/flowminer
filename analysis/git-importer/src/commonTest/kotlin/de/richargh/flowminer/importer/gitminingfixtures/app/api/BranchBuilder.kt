...
class BranchBuilder {
    private var name: BranchId = NamedBranchId.Certain(
        BranchName("feature-branch")
    )
    private var intermediateCommits = mutableSetOf<CommitHash>()
    private var firstCommitHash: CommitHash =
        CommitHash("abc123")
    ...
    fun name(name: String) = apply { this.name = NamedBranchId.Certain(
        BranchName(name)
    ) }
    fun inferredName(name: String) = apply { this.name = NamedBranchId.Inferred(
        BranchName(name)
    ) }
    fun unNamed(tipCommit: String = "unknown") = apply { this.name =
        NamelessBranchId(
    ...
    fun targetBranch(branch: String) = apply { this.targetBranch =
        BranchName(branch)
    }
