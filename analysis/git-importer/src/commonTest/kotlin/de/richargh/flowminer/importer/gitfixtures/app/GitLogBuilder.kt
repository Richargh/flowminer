package de.richargh.flowminer.importer.gitfixtures.app
...
class GitLogEntryBuilder {
    ...
    fun file(path: String, additions: Int = 0, deletions: Int = 0) = apply {
        this.fileChanges += FileChangeEntry(
            path,
            additions,
            deletions
        )
...
class GitLogBuilder {
    ...
    fun isDeletedBranch(branch: String) = apply { deletedBranches.add(
        BranchName(
            branch
        )
    ) }
    ...
    fun anEntry(branch: String, vararg parentBranches: String, block: GitLogEntryBuilder.() -> Unit = {}): CommitHash {
        ...
        val parentBranchNames = parentBranches.map {
            BranchName(
                it
            )
        }
