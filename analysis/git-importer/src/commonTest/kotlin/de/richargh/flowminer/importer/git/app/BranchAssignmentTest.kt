...
import de.richargh.flowminer.importer.gitfixtures.app.api.aCommit
import de.richargh.flowminer.importer.gitfixtures.app.test.haveSameBranchAs
...
class BranchAssignmentTest {
    ...
    fun `should assign Certain branch from tip`() {
        ...
        // Then
        result.first().branchId shouldBe NamedBranchId.Certain(
    ...
    fun `should propagate Certain to first parent`() {
        ...
        result[0] should haveSameBranchAs(
            aCommit {
                certainBranch("origin/main")
                message("Second")
            })
        result[1] should haveSameBranchAs(
            aCommit {
                certainBranch("origin/main")
                message("First")
            })
    ...
    fun `should propagate Certain to grand parents`() {
        ...
        result[2] should haveSameBranchAs(
            aCommit {
                certainBranch("origin/main")
                message("Second")
            })
        result[3] should haveSameBranchAs(
            aCommit {
                certainBranch("origin/main")
                message("First")
            })
    ...
    fun `should prioritize the HEAD branch, when a commit is in multiple possible branches, even when first commit is not on head branch`() {
        ...
        result[4] should haveSameBranchAs(
            aCommit {
                certainBranch("origin/trunk")
    ...
    fun `merge creates separate NamelessBranch for each merged parent`() {
        ...
        mainCommit.branchId shouldBe NamedBranchId.Certain(
            BranchName("origin/main")
        )
    ...
    fun `should assign mixed Certain and null in octopus merge when some branches have tips and merge-message is non-standard`() {
        ...
        result[2].branchId shouldBe NamelessBranchId(
            CommitHash("2")
        )
        result[3].branchId shouldBe NamelessBranchId(
            CommitHash("1")
        )
    }
}