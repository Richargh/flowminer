...
class Branches(branches: List<Branch>) {
    ...
    operator fun get(name: String): Branch? = namedBranches[BranchName(
        name
    )]
...
data class GitMiningResult(
    val commits: Commits,
    val branches: Branches = Branches(
        emptyList()
    ),
    val workItems: WorkItems = WorkItems(
        emptyList()
    ),
    val authorStatistics: AuthorStatistics = AuthorStatistics(
        emptyList()
    )
)