...
private fun parseCommit(raw: RawCommit, tracker: BranchTracker): Commit {
    ...
    val parents = parseParents(
        raw.headerFields["parents"] ?: ""
    )
    val refs =
        parseRefs(raw.headerFields["refs"] ?: "")

    val fileChanges =
        parseFileChanges(raw.files.lines())
...
private fun parseRefs(rawRefs: String): List<Ref> {
    ...
    return rawRefs.split(",").map { it.trim() }.mapNotNull {
        parseRef(
            it
        )
    }
...
private fun parseRef(rawRef: String): Ref? {
    return when {
        rawRef.startsWith("HEAD -> ") -> Ref.LocalHead(
            BranchName(rawRef.removePrefix("HEAD -> "))
        )
