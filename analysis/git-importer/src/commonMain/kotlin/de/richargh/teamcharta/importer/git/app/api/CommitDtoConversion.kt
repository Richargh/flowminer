...
import de.richargh.teamcharta.model.*
...
fun FileChange.toDto(): SerializableFileChangeDto = SerializableFileChangeDto(
    path = path,
    additions = additions,
    deletions = deletions,
    isRename = isRename,
...
fun CommitDto.toCommit(): Commit = Commit(
    ...
    workKeys = workKeys.map { key ->
        if (key == null) WorkKey.Unknown else WorkKey.Known(key)
    },
...
fun BranchIdDto.toBranchId(): BranchId = when (type) {
    "certain" -> NamedBranchId.Certain(BranchName(name!!))
    "inferred" -> NamedBranchId.Inferred(BranchName(name!!))
    "nameless" -> NamelessBranchId(CommitHash(tipCommit!!))
    else -> throw IllegalArgumentException("Unknown branch type: $type")
}

fun FileChangeDto.toFileChange(): FileChange = FileChange(
    path = path,
    additions = additions,
    deletions = deletions,
    isRename = isRename,
